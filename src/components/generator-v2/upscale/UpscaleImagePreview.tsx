  ...
  return (
    <div className="flex-1 flex items-center justify-center p-4 md:p-8 bg-[#0A0A0A]">
      <div className="relative w-full max-w-4xl">
          ...
          {isProcessing && (
            <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
              <div className="flex flex-col items-center gap-4">
                <Loader2 className="w-12 h-12 text-[#CDFF00] animate-spin" />
        ...
        {/* Result label */}
        {hasResult && (
          <div className="absolute top-4 left-4 px-3 py-1.5 rounded-full bg-[#CDFF00]/90 text-black text-sm font-semibold">
            Готово
          </div>
        )}
        ...
        {hasResult && !isProcessing && (
          <div className="flex items-center justify-center gap-3 mt-4">
            {onDownload && (
              <button
                onClick={onDownload}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#CDFF00] text-black font-medium hover:bg-[#B8E600] transition-colors"
              >
