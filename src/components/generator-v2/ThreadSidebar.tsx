            ...
            <div className="p-4 border-b border-[#27272A]">
              ...
              <div className="mt-3 flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 h-10 px-3 rounded-lg bg-[var(--gold)] text-black font-semibold text-sm hover:bg-[var(--gold-hover)] transition-colors relative z-10 w-full justify-center"
                  title="Новый проект"
                  type="button"
                >
                ...
                <div className="space-y-1">
                  {threads.map((t) => {
                    ...
                    return (
                      ...
                      >
                        <MessageSquare className={cn("w-4 h-4", active ? "text-[var(--gold)]" : "text-[#71717A]")} />
                        {isEditing ? (
                          <div className="flex-1 min-w-0 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                            <input
                              value={draftTitle}
                              onChange={(e) => setDraftTitle(e.target.value)}
                              className="flex-1 min-w-0 bg-[#0F0F10] border border-[#3A3A3C] rounded-md px-2 py-1 text-sm text-white outline-none focus:border-[var(--gold)]"
                              maxLength={80}
                              autoFocus
                            />
                            <button
                              type="button"
                              className="inline-flex items-center justify-center w-8 h-8 rounded-md bg-[var(--gold)] text-black hover:bg-[var(--gold-hover)] transition-colors disabled:opacity-60"
                              onClick={saveEdit}
