...
export function ImageGalleryMasonry({ 
  ...
  const cardClassName =
    layout === "grid"
      ? "relative group rounded-sm overflow-hidden bg-[#27272A] cursor-pointer transition-colors"
      : "relative group rounded-sm overflow-hidden bg-[#27272A] cursor-pointer transition-transform hover:scale-[1.02] hover:shadow-2xl";
  ...
  const renderSkeletons = () => {
    return Array.from({ length: 4 }).map((_, i) => (
      <div
        key={`skeleton-${i}`}
        className="relative aspect-square rounded-sm overflow-hidden bg-[#27272A] animate-pulse"
      >
