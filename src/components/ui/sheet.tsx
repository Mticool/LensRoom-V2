...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[--color-background-secondary] p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-[--color-background-primary] transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[--color-gold] focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-5 w-5 text-[--color-text-secondary]" />
