...
-- Migration: 023_referral_system.sql
-- =====================================================
...
CREATE INDEX idx_referral_attributions_code ON public.referral_attributions(code);
...
CREATE TABLE IF NOT EXISTS public.referral_events (
  ...
  referrer_user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  event_type text NOT NULL, -- 'signup', 'first_generation', etc.
  event_key text NOT NULL UNIQUE, -- для идемпотентности: 'signup:<invitee_id>', 'first_gen:<gen_id>'
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamptz DEFAULT now()
);

CREATE INDEX idx_referral_events_invitee ON public.referral_events(invitee_user_id);
...
CREATE INDEX idx_referral_events_key ON public.referral_events(event_key);
...
CREATE TABLE IF NOT EXISTS public.referral_rewards (
  ...
  reward_type text NOT NULL DEFAULT 'stars', -- 'stars', 'credits', etc.
  amount integer NOT NULL,
  reason text NOT NULL, -- описание причины награды
...
CREATE INDEX idx_referral_rewards_event ON public.referral_rewards(event_id);
...
CREATE INDEX idx_affiliate_applications_user ON public.affiliate_applications(user_id);
...
CREATE INDEX idx_affiliate_tiers_tier ON public.affiliate_tiers(tier);
...
-- referral_codes: пользователи могут видеть только свой код
ALTER TABLE public.referral_codes ENABLE ROW LEVEL SECURITY;

CREATE POLICY referral_codes_select_own ON public.referral_codes
  FOR SELECT USING (auth.uid() = user_id);

-- referral_attributions: пользователи могут видеть свои атрибуции
...
-- referral_events: пользователи могут видеть события, где они участвуют
...
-- referral_rewards: пользователи могут видеть только свои награды
ALTER TABLE public.referral_rewards ENABLE ROW LEVEL SECURITY;

CREATE POLICY referral_rewards_select_own ON public.referral_rewards
  FOR SELECT USING (auth.uid() = user_id);

-- affiliate_applications: пользователи могут видеть только свои заявки
...
-- affiliate_tiers: пользователи могут видеть только свой tier
