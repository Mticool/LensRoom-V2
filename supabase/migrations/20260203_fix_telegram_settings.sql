-- Migration: Fix telegram_user_settings table
-- Date: 2026-02-03
-- Purpose: Add missing columns or create table if not exists

-- Drop existing table and recreate (CAREFUL: this will delete data)
-- Only use if you don't have important data
DROP TABLE IF EXISTS telegram_favorites CASCADE;
DROP TABLE IF EXISTS telegram_user_settings CASCADE;
...
CREATE INDEX idx_telegram_user_settings_telegram_id
  ON telegram_user_settings(telegram_id);

-- Index for finding users with active conversations
CREATE INDEX idx_telegram_user_settings_conversation
  ON telegram_user_settings(conversation_expires_at);

-- ========================================
-- Table: telegram_favorites
-- ========================================
CREATE TABLE telegram_favorites (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_telegram_favorites_telegram_id
